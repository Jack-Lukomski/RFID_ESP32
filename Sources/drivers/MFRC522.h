/**
 * @file MFRC522.h
 * @brief MFRC522 RFID Module
 * @author Jack Lukomski
 * @date Date: 2023-06-02
 *
 * This file contains the declarations for the MFRC522 RFID module. It provides
 * functions for initializing, communicating, and authenticating with MIFARE
 * RFID cards using the MFRC522 chip.
 */
...
/** @typedef uidSize_t
 *  @brief Enum typedef for UID size. It can be either fourBytesSingle,
 * sevenBytesDouble or tenBytesTripple.
 */
typedef enum {
...
typedef struct {
  ...
  uint8_t sakByte; /**< The SAK (Select acknowledge) byte returned from the PICC
...
#define PCD_CMD_NO_CMD_CHANGE                                                  \
  0x07 // Can be used to modify the CommandReg register bits without affecting
       // the command, if any, currently being executed
#define PCD_CMD_RECEIVE 0x08 // Activates the receiver circuits
#define PCD_CMD_TRANSCEIVE                                                     \
  0x0C // Transmits data from FIFO buffer to antenna and automatically activates
       // the receiver after transmission
#define PCD_CMD_MF_AUTHENT                                                     \
 ...
 * @param resultBuf Pointer to the buffer to store the calculated CRC value (2
 * bytes).
 * @return ESP_OK if successful, otherwise an error code.
 */
 ...
 * @return Pointer to the UniqueIdentifier_t structure containing the UID
 ...
 * @return Pointer to the Mifare1kKey_t structure containing the key data, or
 * NULL on failure.
 */
...
/**
 * @brief Writes the provided data to the specified block address for the
 * specified UID in the MFRC522.
 *
 * @param spiHandle Pointer to the SPI device handle.
...
/**
 * @brief Sets the SAK (Select Acknowledge) byte for the specified UID in the
 * MFRC522.
 *
 * @param spiHandle Pointer to the SPI device handle.
 * @param UID Pointer to the UniqueIdentifier_t structure containing the UID.
...
/**
 * @brief Reads the key block at the specified block address for the specified
 * UID in the MFRC522.
 *
 * @param spiHandle Pointer to the SPI device handle.
...
/**
 * @brief Performs a block check character calculation for the provided data and
 * UID.
 *
...
/**
 * @brief Performs MIFARE authentication for the specified block address and key
 * with the MFRC522.
 *
 * @param spiHandle Pointer to the SPI device handle.
 * @param cmd The command for the authentication (PICC_CMD_MF_AUTH_KEY_A or
